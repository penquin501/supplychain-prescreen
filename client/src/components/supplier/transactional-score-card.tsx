...
export default function TransactionalScoreCard({
  ...
  const calculateTransactionalScore = (transactions: Transaction[]) => {
    if (!transactions || transactions.length === 0)
      return { score: 50, recency: 3, frequency: 1, monetary: 1 };

    // Recency (1-5): How recently they made transactions
    const latestTransaction = Math.max(
      ...transactions.map((t) =>
        new Date(t.paymentDate || t.invoiceDate || t.poDate).getTime()
      )
    );
    const daysSinceLatest = Math.floor(
      (Date.now() - latestTransaction) / (1000 * 60 * 60 * 24)
    );
    const recency =
      daysSinceLatest <= 30
        ? 5
    ...
    const totalValue = transactions.reduce(
      (sum, t) => sum + parseFloat(t.poNetAmount),
      0
    );
    const monetary =
      totalValue >= 50000000
  ...
  const avgPaymentTerm = transactions?.length
    ? Math.round(
        transactions.reduce((sum, t) => sum + t.poPaymentTerm, 0) /
          transactions.length
      )
    : 0;
          ...
          <div className="flex justify-between">
            ...
            <span
              className={`font-medium ${
                avgPaymentTerm <= 180 ? "text-green-600" : "text-red-600"
              }`}
            >
